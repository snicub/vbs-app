-- 0023_anomaly_grace_period.sql
--
-- Grace period for the "never checked out" alert (is_in_but_not_out).
--
-- The other three anomaly flags all carry a buffer before they fire
-- (is_late_am +45m, is_boarded_but_not_arrived +30m, is_pm_van_stuck +2h),
-- but is_in_but_not_out fired the instant the PM bell passed — so at the exact
-- PM start every still-present kid flashed a red "Never checked out" critical,
-- burying the coordinator in noise during the busiest window of the day.
--
-- Fix: add a 30-minute grace period to the PM-start deadline, mirroring how
-- is_late_am adds +45m to the scheduled AM time. The flag now fires only once a
-- kid is still checked in 30 minutes past PM start, which is a real "should have
-- left by now" signal rather than a bell-time false alarm. The zone stays
-- hard-coded to America/Chicago (the 0020 fix); nothing else in the view changes.
--
-- A view must be redefined whole, so the rest of the body is reproduced verbatim
-- from 0020; only the is_in_but_not_out deadline changes.

-- 0005_student_day_status_view.sql
-- Derived per-(student, day) status. App reads use this view, never the raw log.
...
select
  ...
  -- wristband color: afternoon stop → morning stop → null (UI shows 'P')
  ...
  case
    when r.attending
     ...
     and r.mode in ('van', 'parent_pickup_only')      -- modes that include van AM
...
comment on view public.student_day_status is
  'Derived per-(student,day) status. All app reads use this view — never the raw event log.';

...
description: Post-rework registration collects address (not stop); van kids get NULL stop ids and the downstream view-NULL cascade (no van/color/scheduled-time, is_late_am dead)
metadata:
  type: project
---
...
- Transport in the form = a **single "rides van" checkbox per child** (`ridesVan`). The form inlines the mapping: `transport: { mode: s.ridesVan ? "van" : "parent_both" }` (`signup-form.tsx:121`). **There is NO two-checkbox vanAm/vanPm UI and NO `deriveMode`/`deriveTransportMode` helper** — `src/lib/registration/transport.ts` was deleted (dead-code sweep). The 4 modes still exist in the enum/domain (`parent_dropoff_only`/`parent_pickup_only` are reachable via the coordinator edit screen + historical data), but signup only ever emits `van` or `parent_both`.
- `registerFamily` writes `student_day_records` with **`morning_stop_id = null`, `afternoon_stop_id = null` for everyone** (van kids included) — `registration.ts:209-210`. Van kids are "not yet assigned to a van"; the coordinator resolves this via address→van assignment.

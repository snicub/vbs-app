...
export async function generateMetadata({
  ...
  const { data } = await supabase
    .from("students")
    .select("legal_first_name, legal_last_name, preferred_first_name")
    .eq("id", studentId)
    .maybeSingle<{ legal_first_name: string; legal_last_name: string; preferred_first_name: string | null }>();
  const name = data
    ? `${data.preferred_first_name ?? data.legal_first_name} ${data.legal_last_name}`
    : "Student";
...
export default async function StudentEditPage({
  ...
  const displayName = student.preferred_first_name ?? student.legal_first_name;
  ...
  return (
    <main className="mx-auto max-w-2xl px-3 sm:px-4 py-4 sm:py-6 space-y-6">
      ...
      >
        <ArrowLeftIcon className="size-4" /> Back to {displayName}
      </Link>
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="space-y-0.5 min-w-0 flex-1">
          <h1 className="text-xl sm:text-2xl font-semibold truncate">
            Edit: {displayName} {student.legal_last_name}
          </h1>
          <div className="text-sm text-muted-foreground">
            <code className="font-mono">{student.wristband_code}</code>
            <span className="mx-1.5">·</span>
            Legal name: {student.legal_first_name} {student.legal_last_name}
          </div>
        </div>
      </div>
      ...
      <StudentEditForm
        ...
        eventDate={today}
        initialPreferredFirstName={student.preferred_first_name ?? ""}
